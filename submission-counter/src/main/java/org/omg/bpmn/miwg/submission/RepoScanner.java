...
public class RepoScanner {
    ...
    protected static final Logger LOGGER = Logger
            .getLogger(RepoScanner.class.getName());
    ...
    public RepoScanner() {
        ...
        // backward compatibility for pre Github actions
    ...
    private List<String> getReferences(GHRepository repository)
            throws IOException {
        ArrayList<String> references = new ArrayList<String>();
        List<GHContent> directoryContent = repository
                .getDirectoryContent("Reference");
        for (GHContent content : directoryContent) {
            String name = content.getName();
            String type = content.getType();
            if ("file".equals(type)
                    && name.matches("[A-C]\\.[1-9][0-9]*\\.[0-9]+.bpmn")) {
    ...
    private JSONObject getSubmissionsFromRootDir(GHRepository repository)
            throws IOException {
    ...
    private void getSubmissionsFromToolDir(JSONObject submissions,
            GHRepository repository, String tool) throws IOException {
        ...
        for (GHContent content : directoryContent) {
            ...
            for (String testCase : testCases) {
                if ("file".equals(type)) {
                    if (name.matches(testCase
                            + "-(import.png|roundtrip.bpmn|export.png|export.bpmn)")) {
                        count++;
                        ...
                        LOGGER.warning("[" + tool + "] File '" + name
                                + "' does not match the naming conventions.");
                    }
                }
            }
        }
