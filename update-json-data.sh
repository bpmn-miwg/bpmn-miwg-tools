...
wget "https://api.github.com/repos/bpmn-miwg/bpmn-miwg-test-suite/issues?state=open&per_page=100&page=5"   --output-document=issues-open-05.json
...
echo "  }," >> issues-open.json
tail -n +2 issues-open-04.json | head -n -2 >> issues-open.json
echo "  }," >> issues-open.json
tail -n +2 issues-open-05.json >> issues-open.json
rm issues-open-*.json
